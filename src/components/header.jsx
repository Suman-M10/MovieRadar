...
const Header = () => {
  ...
  const activeClass = "block py-2 pr-4 pl-3  text-white rounded bg-blue-700 md:bg-transparent md:text-blue-700  md:p-0 dark:text-white"
  ...
  const handleSubmit = (e) => {
    ...
    Navigate(`/search/movie/${e.target.search.value}`);
    e.target.search.value = '';
  }
  return (
    <header>
      <nav className="bg-white border-b-2 border-gray-200 px-2 sm:px-4 py-2 dark:border-gray-800 dark:bg-gray-900">
        <div className="container flex flex-wrap justify-between items-center mx-auto">
          <Link to="/" className="flex items-center">
            <img src={Logo} className="mr-3 h-6 sm:h-9" alt="Cinemate Logo" />
            <span className="self-center text-xl font-semibold whitespace-nowrap dark:text-white">MovieRadar</span>
          </Link>
          <div className="flex md:order-2">
            <button onClick={() => theme == "dark" ? setTheme("light") : setTheme("dark")} className="text-gray-500 dark:text-gray-400 hover:bg-gray-100 ring dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm px-2 mr-2 transition-all duration-300 ease-in-out">
              {theme == "dark" ? <img src={sun} className="h-5 w-5" /> : <img src={moon} className="h-5 w-5" />}
            </button>
            <button onClick={() => setHidden(!hidden)} type="button" data-collapse-toggle="navbar-search" aria-controls="navbar-search" aria-expanded="false" className="md:hidden text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-2.5 mr-1 transition-all duration-300 ease-in-out">
